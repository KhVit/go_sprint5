...
func (t Training) meanSpeed() float64 {
	...
	speedKmInHour := t.distance() / t.Duration.Hours() // t.Duration / MinInHours		???
	return speedKmInHour
}
...
func (r Running) Calories() float64 {
	...
	calRun := ((CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Training.Weight / MInKm * r.Duration.Hours() * MinInHours)
	return calRun
}
...
func (r Running) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	infoRun := r.Training.TrainingInfo()
	//infoRun.Calories = r.Calories()
	return infoRun
}
...
func (w Walking) Calories() float64 {
	...
	calWalk := ((CaloriesWeightMultiplier*w.Training.Weight + (math.Pow(meanSpeedMsec, 2)/heightM)*
...
func (w Walking) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	infoWalk := w.Training.TrainingInfo()
	//infoWalk.Calories := w.Calories()
	return infoWalk
}
...
func (s Swimming) meanSpeed() float64 {
	...
	speedSwim := float64(s.LengthPool*s.CountPool) / MInKm / s.Training.Duration.Hours()
	return speedSwim
}
...
func (s Swimming) Calories() float64 {
	...
	calSwim := float64((s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Training.Weight * s.Training.Duration.Hours())
	return calSwim
}
